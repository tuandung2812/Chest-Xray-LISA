...
def initialize_environment(args):
    ...
    wandb.init(project=args.exp_name, config=args)  # Initialize wandb with the project name and arguments
...
def train(active_datasets, model, epoch, scheduler, writer, dataset_iters, args, step_choices):
    ...
    def log_progress():
        ...
        if global_step % args.print_freq == 0:
            ...
            if args.local_rank == 0:
                ...
                # Log to WandB
                wandb.log({
                    f"train/{key}": tracker.avg for key, tracker in trackers.items()
                })
                wandb.log({
    ...
    for global_step in range(args.steps_per_epoch):
        ...
        # Log learning rate to WandB and TensorBoard
        if global_step != 0:
