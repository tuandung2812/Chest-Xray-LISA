...
'adversarial_anatomy_open':  "The location of {anatomy} is at <seg>. It has no abnormalities",
    ...
    # 'adversarial_finding_open': "The location of {anatomy}
...
if __name__ == "__main__":
    ...
    def get_all_json_paths(folder_path):
        json_paths = []
        # print('walk: ' ,os.walk(folder_path))
        i = 0
        for root, _, files in tqdm(os.walk(folder_path)):
            ...
            # print(files)
            for file in files:
                if file.lower().endswith(".json"):  # Case-insensitive check for .json
        ...
        # Sử dụng
    folder_path = "./dataset/MIMIC_MedGLaMM_caption_v3/"
    json_files = get_all_json_paths(folder_path)
    # print(txt_files)
    ...
    for file_path in tqdm(json_files):
        ...
        # print(file_path)
        image_name = '/'.join(file_path.split('/')[-4:]).replace('.json','.jpg')
        ...
        for impression in data_dict['impression']:
            ...
            all_anatomies.append(impression['anatomy']['anatomy_name'])
            disease_mappings[disease_name] = [impression['anatomy']['anatomy_name']]
            if impression['anatomy']['anatomy_name'] not in segment_anat:

                    segment_anat.append(impression['anatomy']['anatomy_name'])

            else:
                if impression['anatomy']['anatomy_name'] not in non_segment_anat:
                    non_segment_anat.append(impression['anatomy']['anatomy_name'])
    ...
    # print('Non segment: ', non_segment_anat)
    # print('segment: ', segment_anat)
    ...
    for file_path in tqdm(json_files):
        ...
        for impression in data_dict['impression']:
            # print(impression['anatomy'].keys())
            # print(len(impression['anatomy']))
            disease_name = impression['disease']['disease_name']

            if disease_name not in disease_mappings:
                disease_mappings[disease_name] = []

            # for anatomy in impression['anatomy']:
                # print(anatomy['anatomy_name'])
            disease_mappings[disease_name].append(impression['anatomy']['anatomy_name'])
        ...
        if disease_mappings:
            ...
            for disease in positive_diseases:
                ...
                grounded_answer = f"The location of {pos_anat} is at [SEG]. It suffers from"
                ...
                for i in range(len(anat_mappings[pos_anat])):
                    ...
                    if len(anat_mappings[pos_anat]) == 1:
                        ...
                        grounded_answer += f' and {disease}.'
                        answer +=  f" and {disease}."
...
#                                        "grounded_answer": grounded_answer_easy, 'anat': easy_anat})
    ...
    with open('process_mimic/mimic_qa_data_train.json', 'w', encoding='utf-8') as json_file:
        json.dump(train_data, json_file, ensure_ascii=False, indent=4)

    with open('process_mimic/mimic_qa_data_test.json', 'w', encoding='utf-8') as json_file:
