...
import numpy as np
import torch, os
from cxas.label_mapper import label_mapper, colors, colors_alpha
...
def visualize_mask(class_names: list, 
    ...
    label_to_visualize = np.concatenate([np.array(category_ids[n]).flatten() for n in class_names]).flatten()
...
def visualize_from_file(class_names: list, 
    ...
    visualization = visualize_mask(class_names, label, img, img_size, cat, axis, category_ids)
    if do_store:
