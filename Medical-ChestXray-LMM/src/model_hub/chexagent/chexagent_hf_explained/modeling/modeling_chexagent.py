...
class CheXagentForConditionalGeneration(CheXagentPreTrainedModel):
    ...
    def generate(
        ...
        if pixel_values is not None:
            ...
            # step 3: project vision to language
            input_vis = self.language_projection(query_output)  # [1, 128, 4096]
            vis_atts = torch.ones(input_vis.size()[:-1], dtype=torch.long, device=input_vis.device)  # [1, 128]
        ...
        inputs_lang = self.language_model.get_input_embeddings()(input_ids)  # [1, 34, 4096]
        lang_atts = attention_mask  # [1, 34]
        if lang_atts is None:
            lang_atts = torch.ones_like(input_ids)

        # step 5: conditioned on the images and/or prompts
        if pixel_values is not None:
            inputs_embeds = torch.cat([input_vis, inputs_lang], dim=1)  # [1, 162, 4096]
            attention_mask = torch.cat([vis_atts, lang_atts], dim=1)  # [1, 162]
        else:
            inputs_embeds = inputs_lang
            attention_mask = lang_atts
