...
from model_hub.lxmert.lxmert_0_explained.build_lxmert_0_explained import lxmert_0_explained_model_registry
...
class LXMERTExplainedHandler:
    def __init__(self, context):
        self.initialize(context)

    def initialize(self, context):
        self.model = lxmert_0_explained_model_registry[context.model_id](
            use_lrp=context.use_lrp
        )
        self.use_lrp = context.use_lrp
    ...
    def handle(
        ...
        kwargs = {"alpha": 1}
        output = self.model.forward(input).question_answering_score  # (1, 3129)
        model = self.model.model

        # initialize relevancy matrices
        text_tokens = self.model.text_len  # =17
        image_bboxes = self.model.image_boxes_len  # =36

        # text self attention matrix
        self.R_t_t = torch.eye(text_tokens, text_tokens).to(model.device)  # (17, 17)
        # image self attention matrix
        self.R_i_i = torch.eye(image_bboxes, image_bboxes).to(model.device)  # (36, 36)
        # impact of images on text
        self.R_t_i = torch.zeros(text_tokens, image_bboxes).to(model.device)  # (17, 36)
        # impact of text on images
        self.R_i_t = torch.zeros(image_bboxes, text_tokens).to(model.device)  # (36, 17)


        if index is None:
            index = np.argmax(output.cpu().data.numpy(), axis=-1)  # =1403

        one_hot = np.zeros((1, output.size()[-1]), dtype=np.float32)  # (1, 3129)
        one_hot[0, index] = 1
        one_hot_vector = one_hot  # (1, 3129)
