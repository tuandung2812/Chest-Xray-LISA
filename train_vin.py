...
#         if match:
#             block_num = int(match.group(2))  # Lấy số block
#             if block_num > 15:  # Kiểm tra nếu block_num lớn hơn 10
#                 param.requires_grad = True  # Thiết lập requires_grad=True nếu block_num > 10
        
#         match = re.search(r'vision_tower\.vision_model\.encoder\.layers\.(\d+)', name)
#         if match:
#             block_num = int(match.group(1))  # Lấy số block
#             if block_num > 12:  # Kiểm tra nếu block_num lớn hơn 10
#                 param.requires_grad = True  # Thiết lập requires_grad=True nếu block_num > 10

        freeze_img_encoder_layers = []
        freeze_vision_tower_layers = []
        
#         for name, param in model.named_parameters():
#             if 'image_encoder' in name:
#                 param.requires_grad = True
#             if 'vision_tower.vision_model' in name:
#                 param.requires_grad = True
...
def train(
    ...
    for global_step in range(args.steps_per_epoch):
        ...
        if not args.disable_wandb:
            wandb.log({'train/loss': losses.avg,'train/ce_loss' : ce_loss, 'train/mask_bce_loss' : mask_bce_loss, 'train/mask_dice_loss': mask_dice_loss, 'train/mask_loss': mask_loss})
...
def validate(val_loader, model_engine, epoch, writer, args):
    ...
    for input_dict in tqdm.tqdm(val_loader):
        ...
        if not args.disable_wandb and idx == 0 and not args.eval_only:
            ...
            conversation_table = wandb.Table(columns=["question/answer"], data=[input_dict["conversation_list"]])
            wandb.log({"Conversation": conversation_table}) 
